...
<!-- 二、区域信用评价分析-->
    <div class="part">
        <h3 class="title_style_3">${model_number[model_index]}、区域信用评价分析</h3>
        <div class="flex step_block_wrap j-center">
            <div class="step_block_item flex a-center">
                <div class="step_block_item_flex">
                    <div class="step_block_item_title">区域信用等级</div>
                    <h4 class="blue">${data.baseInfo.areaRank !"-"}</h4>
                </div>

            </div>
            <div class="step_block_item flex a-center">
                <div class="step_block_item_flex">
                    <div class="step_block_item_title">区域信用得分</div>
                    <h4 class="blue">${data.baseInfo.areaScore!"-"}</h4>
                </div>
            </div>
        </div>
        <div class="des"><p>${data.baseInfo.areaDesc!"-"}</p></div>
        <#if (data.baseInfo.areaRank) ??>
        <div>
            <!-- 区域信用评价分析 表格-->
                ...
                <#if data.areaRatingAnalysis ??>
                    ...
                    <tr>
                        ...
                        <#list item.ranges as rangeItem>
                            ...
                            <#if (rangeItem_index > 0) && (rangeItem_index % 5 == 0) ></tr>
                            <tr></#if>
        ...
        <div style="margin-top: 10px;margin-bottom: 10px;">
            <div>备注</div>
            <p>1.“是否区域内公开发债城投企业有息债务余额为0且一般公共预算收入小于100亿元”指标值若为“1.00”则代表“命中”；指标值若为“0.00”则代表“未命中”</p>
            <p>2.“融资压力指标”的指标值即被评价区域的一般公共预算收入，指标落入区间及得分的判定以被评价区域的一般公共预算收入作为判定标准；</p>
        </div>
            ...
            <div class="part_avoid">
                ...
                <table class="statement_list">
                    ...
                    <tr>
                        ...
                        <td>${(item.city)!"-"}</td>
                        <td>${(item.county)!"-"}</td>
                        <td style="text-align:left;"><#if item.content1 ? length gt 80 >${item.content1?substring(0,80)}
                            ...<#else>${item.content1!}</#if></td>
                    </tr>
                    </#list>
                    </tbody>
                </table>
            ...
            <!-- 区域信用评价分析 指标释义-->
             ...
             <div class="part_avoid">
                <#assign t0 = data.baseInfo.T0?number>
                <#assign t1 = data.baseInfo.T1 ?number>
